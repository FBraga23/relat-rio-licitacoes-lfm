...
class ClienteHttp:
    ...
    def _sessao(self) -> requests.Session:
        if not hasattr(self._local, "sessao"):
            self._local.sessao = criar_sessao()
...
def coletar(
    ...
    # A consulta principal passa a ser o Dados Abertos do Compras.gov.
    # O endpoint nacional do PNCP fica como fallback, pois pode apresentar
    # timeouts mesmo em janelas muito curtas.
    for nome, codigos in MODALIDADES.items():
        ...
        try:
            ...
            logging.warning(
                "Falha no Compras.gov para %s; tentando PNCP como contingencia: %s",
                nome, exc_compras,
            )
            try:
                ...
                falhas_modalidade += 1
                aviso = (
                    f"Falha ao consultar {nome} nas duas fontes publicas. "
    ...
    def registrar_falha_essencial(rotulo: str, detalhe: Exception) -> None:
        ...
        aviso = f"{rotulo} indisponivel apos retentativas: {detalhe}"
        logging.error(aviso)
        resultado.avisos.append(aviso)
    ...
    def buscar_itens_robusto(compra: dict[str, Any]) -> list[dict[str, Any]]:
        # Para itens, prioriza o endpoint transacional do PNCP por contratação.
        # Ele respondeu de forma estável nos testes reais da UASG e evita uma
        # limitação conhecida do endpoint 2.1 do Compras.gov, que em alguns
        # registros pode retornar apenas parte dos itens. O Compras.gov fica
        # como contingência, usando a sintaxe atual tipo=idCompra&codigo=... .
        ...
        try:
            ...
            logging.warning(
                ...
                compra.get("numeroControlePNCP") or compra.get("idCompra"), exc,
            )
        ...
        # As duas fontes responderam sem erro, mas sem itens. Isso pode ocorrer
        # em registros ainda não materializados nas bases analíticas.
        return []
    ...
    with ThreadPoolExecutor(max_workers=max(1, min(max_workers, 6))) as executor:
        ...
        for futuro in as_completed(futuros):
            ...
            try:
                itens_por_chave[chave] = futuro.result()
            except ApiError as exc:
...
def gerar_pdf(
    ...
    if not resultado.dados_completos:
        ...
        historia.append(
            Paragraph(
                ...
                f"Falha(s) de integridade: {falhas}. O envio automático por e-mail deve permanecer bloqueado.",
                estilos["alerta"],
            )
        )
    ...
    if resultado.cache_pncp_utilizado:
        ...
        historia.append(
            Paragraph(
                ...
                f"Recortes em contingência: {fontes}.",
    ...
    if notas:
        historia.append(
            Paragraph("Observacoes: " + " ".join(notas), estilos["nota"])
        )
    ...
    for situacao in ORDEM_SITUACOES:
        ...
        historia.append(Paragraph(f"{titulo_secao} ({len(processos)})", estilos["secao"]))
        linhas = [
            [
        ...
        for processo in processos:
            data_texto = f"{processo.rotulo_data}:<br/>{formatar_data(processo.data_relevante)}"
            linhas.append(
                [
                    Paragraph(processo.numero_exibicao, estilos["celula"]),
                    Paragraph(processo.objeto, estilos["celula"]),
                    Paragraph(data_texto, estilos["celula"]),
                    Paragraph(processo.situacao_gerencial, estilos["celula"]),
                ]
            )
        tabela = LongTable(
            linhas,
